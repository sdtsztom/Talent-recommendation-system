...
public class IDToDescDAOImpl {
    ...
    public List<Map> degree() throws Exception{
        CachedRowSetImpl rs = CommonConnection.makeQuery("select deg_id,deg_name from degree",ConnectUser.DEV);
        return JsonUtils.toMap(rs,"deg_id","deg_name");
    }

    public List<Map> university() throws Exception {
        CachedRowSetImpl rs = CommonConnection.makeQuery("select uni_id,uni_name from university",ConnectUser.DEV);
        return JsonUtils.toMap(rs,"uni_id","uni_name");
    }

    public List<Map> recommendfrom() throws Exception {
        CachedRowSetImpl rs = CommonConnection.makeQuery("select recf_id,recf_desc from recommend_from",ConnectUser.DEV);
        return JsonUtils.toMap(rs,"recf_id","recf_desc");
    }

    public List<Map> interviewplace() {
        CachedRowSetImpl rs =CommonConnection.makeQuery("select ip_id,work_place.wp_detail,ip_detail from interview_place inner join work_place on wp_id = ip_id;",ConnectUser.DEV);
        try{
    ...
    public List<Map> stuff_type() throws Exception{
        CachedRowSetImpl rs = CommonConnection.makeQuery("select st_id,st_name,st_desc from stuff_type;",ConnectUser.DEV);
        return JsonUtils.toMap(rs,"st_id","st_name","st_desc");
    }

    public List<Map> work_place() throws Exception {
        CachedRowSetImpl rs = CommonConnection.makeQuery("select wp_id,wp_name,wp_detail from work_place;",ConnectUser.DEV);
        return JsonUtils.toMap(rs,"wp_id","wp_name","wp_detail");
    }

    public List<Map> emergency_degree() throws Exception {
        CachedRowSetImpl rs = CommonConnection.makeQuery("select ed_id,ed_name from emergency_degree;",ConnectUser.DEV);
