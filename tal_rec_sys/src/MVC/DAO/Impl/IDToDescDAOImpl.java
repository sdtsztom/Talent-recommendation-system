...
public class IDToDescDAOImpl {
    ...
    public List<Map> degree() throws Exception{
        ...
        ResultSet rs = CommonConnection.makeQuery("select deg_id,deg_name from degree");
        return JsonUtils.toMap(rs,"deg_id","deg_name");
    }

    public List<Map> university() throws Exception {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select uni_id,uni_name from university");
        return JsonUtils.toMap(rs,"uni_id","uni_name");
    }

    public List<Map> recommendfrom() throws Exception {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select recf_id,recf_desc from recommend_from");
        return JsonUtils.toMap(rs,"recf_id","recf_desc");
    }

    public List<Map> interviewplace() {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs =CommonConnection.makeQuery("select ip_id,work_place.wp_detail,ip_detail from interview_place inner join work_place on wp_id = ip_id;");
        try{
    ...
    public List<Map> stuff_type() throws Exception{
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select st_id,st_name,st_desc from stuff_type;");
        return JsonUtils.toMap(rs,"st_id","st_name","st_desc");
    }

    public List<Map> work_place() throws Exception {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select wp_id,wp_name,wp_detail from work_place;");
        return JsonUtils.toMap(rs,"wp_id","wp_name","wp_detail");
    }

    public List<Map> emergency_degree() throws Exception {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select ed_id,ed_name from emergency_degree;");
        return JsonUtils.toMap(rs,"ed_id","ed_name");
    }

    public List<Map> recruitment_requirements_stage() throws Exception {
        CommonConnection.setConnectUser(ConnectUser.DEV);
        ResultSet rs = CommonConnection.makeQuery("select rrs_id,rrs_desc from recruitment_requirements_stage;");
