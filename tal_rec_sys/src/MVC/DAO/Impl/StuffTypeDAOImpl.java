...
public class StuffTypeDAOImpl implements StuffTypeDAO {
    ...
    public List<Map> getAll() throws SQLException{
        CachedRowSetImpl rs = CommonConnection.makeQuery("select * from stuff_type", ConnectUser.HR);
        return JsonUtils.toMap(rs,"st_id","st_name","st_desc");
    }

    @Override
    public List<Map> getName() throws SQLException {
        CachedRowSetImpl rs = CommonConnection.makeQuery("select st_id,st_name from stuff_type",ConnectUser.HR);
