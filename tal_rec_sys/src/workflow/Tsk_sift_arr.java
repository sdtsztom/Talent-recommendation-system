...
public class Tsk_sift_arr {
    ...
    public static boolean finish(int rrid){
        boolean unfinish_person= CommonConnection.existQuery("select * from recommend where rec_rr_id="+rrid+" and rec_recsta_id="+ RStage.W_ARR_S.toId(), ConnectUser.SYS);
        if(unfinish_person)return false;
        else{
            CommonConnection.Update("update recruitment set rr_sta_id="+ RrStage.W_I1+" where rr_id"+rrid,ConnectUser.SYS);
            return true;
        }
    }
    //**************************************API Function***********************************************
    public static void interview(int rec_id){
        // 更新阶段或(与)结果
        CommonConnection.Update("update recommend set rec_recsta_id="+ RStage.W_I1.toId()+" where rec_id="+rec_id,ConnectUser.SYS);
    }
    ...
    public static void otherneed(Arrangement a){
        ...
        CommonConnection.Update("insert into recommend values("+rr_id_of_otherNeed+","+values[1]+","+values[0]+","+
                RecommendFrom.AFT_SIFT.toId()+","+RrStage.OPEN.toId()+","+new_HR_id+","+RResult.NONE.toId()+")",ConnectUser.SYS);
        // 更新阶段或(与)结果
        CommonConnection.Update("update recommend set rec_recsta_id="+ RStage.FINISH.toId()+
